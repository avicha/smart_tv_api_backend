...
module.exports = class DidiController extends BaseController {
    async get_gift(ctx) {
        ...
        let visit_record = await ctx.smart_tv_db.collection('didi_stat').findOne({ ip: ip, action: 'get_js_config', from: from, t: { $gte: now - 10 * 60 * 1000 }, ua: ua })
        let order
        if (visit_record) {
            let prize_count = {
                '1': 1,
    ...
    async get_stat(ctx) {
        ...
        let start_date = new Date('2017-12-15 00:00:00').getTime()
        let end_date = new Date('2018-01-02 00:00:00').getTime()
